...
with open('C:\\Users\\WIN10\\Desktop\\LEARNING\\my_movies.csv') as f: mymovies = f.read()

# Splitting the data into separate transactions using '\n' operator

mymovies = mymovies.split('\n')

mymovieslist = []

for i in mymovies:

    mymovieslist.append(i.split(','))

allmymovies = [i for item in mymovieslist for i in item]

from collections import Counter

itemfrequencies = Counter(allmymovies)
...
mymoviesseries = pd.DataFrame(pd.Series(mymovieslist))

mymoviesseries = mymoviesseries.iloc[:9835,:] # Removing the last empty transaction

mymoviesseries.columns = ['transactions']

# Creating dummy variables for each item in each transaction, using column names as item names

x = mymoviesseries['transactions'].str.join(sep = '*').str.get_dummies(sep = '*')
...
rules.to_csv("mymoviesassociation.csv", encoding = "utf-8")
...
# We got 12 as the highest lift ratio, so that is the best rule